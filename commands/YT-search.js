...
france({ nomCom: "yts", categorie: "Search", reaction: "🎗️" }, async (dest, zk, commandeOptions) => {
  const { ms, repondre, arg } = commandeOptions;
  const query = arg.join(" ");

  if (!query[0]) {
    repondre("එහි වීඩියෝ ලින්ක් හෝ නම දමන්න..?");
    return;
  }
...
france({
  ...
  if (!arg[0]) {
    repondre("Video download 🎗️ youtube video  link");
    return;
  }
...
_*En cours de téléchargement...*_\n\n`
    ...
    */
    ...
    fileStream.on('finish', () => {
      ...
      zk.sendMessage(origineMessage, { video: { url: `./${filename}` }, caption: "*MADE BY RUSI-MD*", gifPlayback: false }, { quoted: ms });
